...
def load_args():
    ...
    parser.add_argument('--meter_per_pixel', type=float, help='每个像素对应距离,单位m')
    parser.add_argument('--pcd_dir', type=str, help='pcd 存储文件夹路径')
    parser.add_argument('--pic_dir', type=str, help='pic 存储文件夹路径')
    parser.add_argument('--location_path', type=str, help='定位点存储文件路径')
...
def load_pcds(args):
    ...
    for folderName, subfolders, filenames in os.walk(pcd_dir):
        ...
        for filename in names:
            ...
            mask = mask_w & mask_h
            points = points[mask]
            bev_pointcloud_image_np = np.zeros(
                [pic_height, pic_width, 3], np.uint8)
