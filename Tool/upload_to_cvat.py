'''
Description: 
Version: 
Author: Leidi
Date: 2022-09-29 10:40:01
LastEditors: Leidi
LastEditTime: 2022-09-29 10:40:25
'''
import argparse
import json
import os
from pathlib import Path

import requests


def get_token(username, email, password):
    request_url = "http://192.168.1.28:8080/api/auth/login"
    params = {"username": username, "email": email, "password": password}
    response = requests.post(request_url, data=params)
    if response.status_code == 200:
        print(
            json.dumps(response.json(),
                       sort_keys=True,
                       indent=4,
                       separators=(', ', ': '),
                       ensure_ascii=False))
        dictRS = dict(response.json())
        return dictRS['key']
    else:
        print(response)


def upload_to_cvat(opt, project_id):
    data_path = opt.data_path
    path_list = list(Path(data_path).glob(".zip"))
    for path in path_list:
        zip_path = str(path)
        task_name = ""
        os.system(
            f"python3 util/cli/cli.py --auth admin:admin123 --server-host 192.168.1.28 create {task_name} --project_id {project_id} --labels labels.json local {zip_path}"
        )


def get_project_id(project_name):
    username = "admin"
    email = "xiongdaecust@163.com"
    password = "admin123"
    token = get_token(username, email, password)
    headers = {
        'Content-Type':
        'application/json',
        "Authorization":
        f"Token {token}",
        "User-Agent":
        "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/104.0.5112.81 Safari/537.36 Edg/104.0.1293.54"
    }
    rep = requests.get(url="http://192.168.1.28:8080/api/projects",
                       headers=headers)
    projects = json.loads(rep.text)["results"]
    project_ids = {}
    for obj in projects:
        project_ids[obj["name"]] = obj["id"]
    return project_ids[project_name]


if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description='data studio',
        formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    parser.add_argument('--data_path', type=str, default="", help='data path')
    parser.add_argument('--project_name',
                        type=str,
                        default="",
                        help='project_name')
    opt = parser.parse_args()
    project_name = opt.project_name
    project_id = get_project_id(project_name)
    upload_to_cvat(opt, project_id)
