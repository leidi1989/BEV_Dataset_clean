...
LastEditors: Leidi
LastEditTime: 2022-09-02 14:04:59
'''
...
class CVAT_IMAGE_BEV_NAS(Dataset_Base):
    ...
    def __init__(self, dataset_config: dict) -> None:
        ...
        # ftp
        if self.save_in_nas:
            self.ftp_image_root = os.path.join(self.nas_dataset_input_folder,
                                               'sync_data_annotation_data',
                                               'cvat')
            self.ftp_annotation_root = os.path.join(
                self.nas_dataset_input_folder, 'xml_128')

        # local
        self.download_nas_data_folder = check_output_path(
            os.path.join(dataset_config['Dataset_output_folder'],
                         'nas_source_dataset_annotations_images'))
        self.download_nas_data_image_folder = check_output_path(
            os.path.join(self.download_nas_data_folder, 'images'))
        self.download_nas_data_annotation_folder = check_output_path(
            os.path.join(self.download_nas_data_folder, 'annotations'))
        self.source_dataset_annotation_image_folder = check_output_path(
            os.path.join(dataset_config['Dataset_output_folder'],
                         'source_dataset_annotations_images'))
        self.source_dataset_image_form_list = ['jpg', 'png']
        self.source_dataset_annotation_form = 'xml'
        if dataset_config['Save_in_nas']:
            self.source_dataset_image_count = self.download_image_and_get_source_dataset_image_count(
            )
            self.source_dataset_annotation_count = self.download_annotation_and_get_source_dataset_annotation_count(
            )
        else:
        ...
        if self.save_in_nas:
            self.ftp.close()
        # self.source_dataset_image_count = 2484
        # self.source_dataset_annotation_count = 1
        self.camera_image_wh = [0, 0]

    def download_image_and_get_source_dataset_image_count(self) -> int:
        """[获取源数据集图片数量]
        ...
        print(
            '\nStart download nas source dataset image and get source dataset image count:'
        )
        image_count = 0
        self.ftp.cwd(self.ftp_image_root)
        for image_folder in tqdm(
                self.ftp.nlst(),
                desc='Download annotation and camera image from nas'):
            if image_folder.split('.')[-1] == 'zip':
                continue
            local_annotation_camera_image_folder = check_output_path_(
                os.path.join(self.download_nas_data_image_folder,
                             image_folder))
            self.ftp.cwd(image_folder)
            for file in tqdm(self.ftp.nlst(),
                             desc='Download annotation image',
                             leave=False):
                if os.path.splitext(file)[-1].replace('.', '') in \
                            self.source_dataset_image_form_list:
                    local_annotation_image_path = os.path.join(
                        local_annotation_camera_image_folder, file)
                    with open(local_annotation_image_path, 'wb') as f:
                        self.ftp.retrbinary('RETR ' + file, f.write, 2048)
                        f.close()
            self.ftp.cwd('related_images')
            local_camera_image_folder = check_output_path_(
                os.path.join(local_annotation_camera_image_folder,
                             'related_images'))
            for file in tqdm(self.ftp.nlst(),
                             desc='Download camera image',
                             leave=False):
                local_camera_image_one_folder = check_output_path_(
                    os.path.join(local_camera_image_folder, file))
                self.ftp.cwd(file)
                for img in self.ftp.nlst():
                    if os.path.splitext(img)[-1].replace('.', '') in \
                                self.source_dataset_image_form_list:
                        local_camera_image_path = os.path.join(
                            local_camera_image_one_folder, img)
                        with open(local_camera_image_path, 'wb') as f:
                            self.ftp.retrbinary('RETR ' + img, f.write, 2048)
                            f.close()
                        image_count += 1
                    self.ftp.cwd('..')
            self.ftp.cwd('../..')
    ...
    def download_annotation_and_get_source_dataset_annotation_count(
            self) -> int:
        """[获取源数据集标注文件数量]
        ...
        print(
            '\nStart download nas source dataset annotation and get source dataset annotation count:'
        )
        annotation_count = 0
        self.ftp.cwd(self.ftp_annotation_root)
        for file in self.ftp.nlst():
            if file.split('.')[-1] == 'zip' and file.split('.')[0].split(
                    '_')[-1] == 'image':
                local_file_path = os.path.join(
                    self.download_nas_data_annotation_folder, file)
                with open(local_file_path, 'wb') as f:
                    self.ftp.retrbinary('RETR ' + file, f.write, 2048)
                    f.close()
                temp_annotation_zip_file = zipfile.ZipFile(local_file_path)
                for names in temp_annotation_zip_file.namelist():
                    temp_annotation_zip_file.extract(
                        names, self.download_nas_data_annotation_folder)
                temp_annotation_zip_file.close()
                annotation_count += 1
    ...
    def get_source_dataset_image_count(self) -> int:
        ...
        image_count = 0
        self.dataset_input_folder = self.download_nas_data_folder
        for n in os.listdir(os.path.join(self.dataset_input_folder, 'images')):
            image_count += len(
                os.listdir(
                    os.path.join(self.dataset_input_folder, 'images', n,
                                 'related_images')))
    ...
    def source_dataset_copy_image_and_annotation(self) -> None:
        ...
        if not self.only_statistic:
            ...
            for root, dirs, _ in os.walk(self.dataset_input_folder):
                if root != os.path.join(self.dataset_input_folder, 'images'):
                    continue
                for dirct in dirs:
        ...
        for n in os.listdir(
                os.path.join(self.dataset_input_folder, 'annotations')):
            if n == 'annotations.xml':
                annotation_path = os.path.join(self.dataset_input_folder,
                                               'annotations', n)
    ...
    def annotation_check(dataset_instance: Dataset_Base) -> list:
        """[读取CVAT_IMAGE_BEV_NAS数据集图片类检测列表]
